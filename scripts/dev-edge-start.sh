...
TEMPLATE_PATH="$DIR/presence_edge_auth/template.yaml"
...
if command -v sam >/dev/null 2>&1; then
    ...
    cd "$DIR/presence_edge_auth" && devbox run -- sam build --template "$TEMPLATE_PATH"
else
...
BUILT_TEMPLATE="$DIR/presence_edge_auth/.aws-sam/build/template.yaml"

if command -v sam >/dev/null 2>&1; then
    sam local start-lambda --host 0.0.0.0 --port 3343 --template "$BUILT_TEMPLATE"
elif command -v devbox >/dev/null 2>&1; then
    cd "$DIR/presence_edge_auth" && devbox run -- sam local start-lambda --host 0.0.0.0 --port 3343 --template "$BUILT_TEMPLATE"
else
    echo "Error: sam not found. Install AWS SAM CLI or use devbox." >&2
    exit 1
fi
